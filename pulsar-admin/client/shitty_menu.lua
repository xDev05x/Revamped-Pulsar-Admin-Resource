function OpenStaffMenu(data)
    ...
    if data then
        adminSubMenus = {}
        adminMenu = exports['pulsar-menu']:Create('adminMenu', 'Staff Menu', function(id, back)
            _menuOpen = true
        end, function()
            _menuOpen = false
            Wait(100)
        ...
        if data.playerData then
            ...
            if #data.playerData > 0 then
                ...
                for _, player in ipairs(data.playerData) do
                    ...
                    adminSubMenus[playerMenuId] = exports['pulsar-menu']:Create(playerMenuId,
                        string.format('Viewing Player: [%s] %s', player.Source, player.Name))

                    -- PUNISH MENU
                    adminSubMenus[playerMenuId .. '-punish'] = exports['pulsar-menu']:Create(
                    ...
                    adminSubMenus[playerMenuId].Add:Button('Attach To (Spectate)',
                        { disabled = not player.Character, success = _attached }, function(data)
                            if _attached then
                                ...
                                exports["pulsar-core"]:ServerCallback('Admin:AttachToPlayer', player.Source,
        ...
        adminSubMenus['recentDisconnects'] = exports['pulsar-menu']:Create('adminRecentDisconnects',
            'Recent Disconnects')
